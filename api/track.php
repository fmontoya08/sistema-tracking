...
function geo_lookup($ip){
    $resp = @file_get_contents("http://ip-api.com/json/".urlencode($ip)."?fields=status,country,regionName,city,lat,lon");
    if(!$resp) return [null,null,null,null,null];
    $j = json_decode($resp,true);
    if(($j['status']??'')!=='success') return [null,null,null,null,null];
    return [$j['country']??null,$j['regionName']??null,$j['city']??null,$j['lat']??null,$j['lon']??null];
}

// Revisar si existe sesi車n
...
if (!$ex) {
    ...
    $pageviews=0; $disconnected=0;
    $stmt->bind_param('sssssssssissssssddi',
        $site,$session_key,$ip,$ua,$device,$referrer,
        $now,$now,$pageviews,
        $utm_source,$utm_medium,$utm_campaign,$tz,
        $country,$region,$city,$lat,$lon,$disconnected
    );
    $stmt->execute();

} else {
    // Sesi車n existente: actualizar last_seen si no est芍 desconectado
    if ($type !== 'unload') {
...
if ($type==='pageview') {
    $stmt2 = $mysqli->prepare("INSERT INTO pageviews
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
    $stmt2->bind_param('ssssssiiisss',
        $site,$session_key,$url,$title,$referrer,$now,$width,$height,$utm_source,$utm_medium,$utm_campaign,$tz
    );
    $stmt2->execute();
    ...
    // Actualizar duraci車n del 迆ltimo pageview
    ...
    // Marcar sesi車n como desconectada
